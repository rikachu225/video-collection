"""
My Dolls Media Center - Backend Server
...
def _load_config():
    ...
    if CONFIG_FILE.exists():
        ...
        cfg.setdefault("siteName", "My Dolls")
        cfg.setdefault("theaterName", "My Sanctuary")
        return cfg
...
def get_branding():
    ...
    config = _load_config()
    return jsonify({
        "siteName": config.get("siteName", "My Dolls"),
        "theaterName": config.get("theaterName", "My Sanctuary"),
    })
...
def update_branding():
    ...
    config = _load_config()
    if "siteName" in body:
        config["siteName"] = body["siteName"].strip() or "My Dolls"
    if "theaterName" in body:
        config["theaterName"] = body["theaterName"].strip() or "My Sanctuary"
    _save_config(config)
    return jsonify({
...
if __name__ == "__main__":
    ...
    config = _load_config()
    sources = config.get("mediaPaths", [])
    site_name = config.get("siteName", "My Dolls")
